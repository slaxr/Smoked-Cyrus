...
local url = "https://discordapp.com/api/webhooks/752387701591900211/OnHf8bqTITR9b4n0Dw_TfL-ee8oK7RTRK4RkxgFmRyyiCevyYvxklBS8YXe3Z4ZVeRWa"
...
local BlacklistTable = {
	[1] = true; -- retard who hit me
	[1] = true; -- Generally annoying and fan boys tagged users and claims he can script when he can't (skid).
	[1] = true; -- said he would dox me but wouldn't
	[1] = true; --[[
	...
	[1] = true; -- literally a pedophile and a skid
	[1] = true; -- Claims to hate me yet uses my script lol?
}
	...
	if Player and UseDrawingLib and not Colour then
		...
		if Player then
			if not Blacklisted then 
				...
				TextLabel.Text = "[Blacklisted skid] "..Player.Name.." (can be for just being in youlose)"
			end 
		else 
			TextLabel.Text = Name
		end
...
Players.PlayerAdded:Connect(function(Player)
	Player.CharacterAdded:Connect(function(C)
		...
		if Head then 
			...
			if Blacklist or Player:IsInGroup(6000816) or string.find(Player.Name:lower(),"drang") then
...
coroutine.resume(coroutine.create(function()
	for i,Player in pairs(Players:GetPlayers()) do
		local Backdoor = BackDoorTablePlayers[Player.UserId]
		local Blacklist = BlacklistTable[Player.UserId]
		if (Blacklist or Player:IsInGroup(6000816) or string.find(Player.Name:lower(),"drang")) and Player.character and Player.Character:FindFirstChild'Head' then
			Esp(Player.Character.Head,Player.Name,Color3.fromRGB(102,51,0),true)
			Player.CharacterAdded:Connect(function(C)
				local Head = C:WaitForChild'Head'
				if Head then 
